...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
  ...
  if (src->format->BitsPerPixel == 32)
  {
    ...
    for (int i = 0; i < rect_h; ++i)
    {
      ...
      {
        int tt = (dy + i) * dst->w + dx + j;
        int kk = (sy + i) * src->w + sx + j;
        dst_pixels[ tt ] = src_pixels[ kk ];
      }
    }
  }
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h)
{
  if (s->format->BitsPerPixel == 32)
  {
    if (w != 0 || h != 0 || x != 0 || y != 0)
    {
      int sz = w * h * sizeof(uint32_t);
      uint32_t *pixels = malloc( sz );
  ...
  else if (s->format->BitsPerPixel == 8)
  {
    ...
    int tt = w * h * sizeof(uint32_t);
    uint32_t *pixels = malloc( tt );
    ...
    for (int i = 0; i < h; ++i)
    {
      for (int j = 0; j < w; ++j)
      {
        int tt = i * w + j;
        int kk = (y + i) * s->w + x + j;
        pixels[tt] = translate_color(&s->format->palette->colors[src[kk] ] );
      }
    }
    NDL_DrawRect(pixels, x, y, w, h);
